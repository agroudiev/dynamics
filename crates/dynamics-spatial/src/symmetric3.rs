...
impl Symmetric3 {
    ...
    #[cfg(feature = "python")]
    /// Creates a `Symmetric3` from a NumPy array.
    /// # Arguments
    /// * `array` - A 3x3 NumPy array.
    /// # Returns
    /// * A `Symmetric3` instance if successful,
    /// * otherwise raises a `ValueError` if the input is not symmetric or not of shape (3, 3).
