...
use model::inverse_dynamics::inverse_dynamics;
...
fn test_rnea_ur5() {
    ...
    let (model, _) = build_models_from_urdf(path).unwrap();
    let mut data = model.create_data();

    let q = Configuration::from_vec(vec![
        6.11532514,
        -0.07569121,
        -1.4693574,
        -5.14300212,
